...
public:
	...
	{
		allTimers.insert(Shared<T>(_timer));
	}
	FORCEINLINE void RemoveTimer(T* _timer)
	{
		const Shared<T>& _timerPtr = Shared<T>(_timer);
		if (!allTimers.contains(_timerPtr)) return;

		_timerPtr->Stop();
		allTimers.erase(_timerPtr);
	}
...
public:
	...
	{
		Pause();
		isToDelete = true; //TODO check to remove
	}
	void Resume()
	{
		isRunning = true;
