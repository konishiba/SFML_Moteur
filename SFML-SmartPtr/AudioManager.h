...
class AudioManager : public Singleton<AudioManager>
{
	bool isMuted;
	float volume;
	multimap<string, Shared<Sample>> allSamples;

public: 
	FORCEINLINE void RegisterSample(Sample* _sample)
	{
		allSamples.insert(make_pair(_sample->GetPath(), Shared<Sample>(_sample)));
	}
...
public:
	...
	Type* PlaySample(const string& _path, const AudioExtensionType& _type = MP3, const Time& _time = Time())
	{
		...
		for (Iterator _iterator = _activeSamples.first; _iterator != _activeSamples.second; ++_iterator)
		{
			...
			{
				_sample->Play(_time);
				return Cast<Type>(_sample.get());
			}
		}

		_sample = MakeShared<Type>(_finalPath);
		_sample->Play(_time);
